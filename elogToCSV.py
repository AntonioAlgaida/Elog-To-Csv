...
def elogToCSV(path, local_path):
    ...
    csvfile = local_path + '\\Results\\' + path.split('\\')[-1].replace('\\','').replace('.elog','') +'-rx_frames'
    ...
    # now tx_frames
    res = tx_frames
    csvfile = local_path + '\\Results\\' + path.split('\\')[-1].replace('\\','').replace('.elog','') +'-tx_frames'
